...
Model comparison on the held-out test set (192 examples from the 20% of organizations withheld by the group-aware split), at the default 0.5 threshold:
...
These are single-split test-set numbers, not cross-validation. The cross-validation figures are lower and are reported separately below: group-aware 5-fold CV gives F1 = 0.672 ± 0.061, which is the honest generalization estimate. Both are computed in `notebooks/Classification_Analysis.ipynb`; the comparison table is saved to `outputs/tables/classifier_comparison.csv`.

After selecting Logistic Regression and optimizing the decision threshold on the same held-out test set (threshold = 0.558), the final test-set performance is:
...
The grid search selects on average precision: best CV average precision = 0.76, recorded in `results_classifier/report.txt`, which is generated by running the pipeline and is not committed.

Scoring the selected model on F1 instead shows what the grouping costs. Both figures below are computed in `notebooks/Classification_Analysis.ipynb`, which is committed:

| Cross-validation scheme | F1 |
|---|---|
| Standard 5-fold (groups ignored — leaky) | 0.694 ± 0.025 |
| Group-aware 5-fold (GroupKFold by `org_id`) | 0.672 ± 0.061 |

The group-aware figure is the honest generalization estimate, and it is the number to quote when asked how the classifier performs on organizations it has never seen. It is meaningfully below both the 0.85 single-split comparison figure and the 0.91 threshold-tuned test F1 — the latter is tuned on the test set, so that caveat travels with it.
