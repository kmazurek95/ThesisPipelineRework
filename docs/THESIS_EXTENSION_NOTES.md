...
| Classifier | Multinomial Naive Bayes (F1 ≈ 0.73, ROC-AUC ≈ 0.72) | Logistic Regression (F1=0.91, ROC-AUC=0.95) |
| Training labels | ~1,000 hand-labeled | 1,222 hand-labeled |
...
The rebuild uses 1,222 labels versus the thesis's ~1,000; the classification gain reflects the modeling choices (TF-IDF + logistic regression, group-aware cross-validation) and the rebuilt dataset, not labels alone.
