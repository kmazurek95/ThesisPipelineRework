...
This script extracts mentions from normalized legislative data.
...
## Arguments:
- `normalized`: Subcommand to process normalized data.
- `--normalized-dir`: Directory containing normalized data.
- `--out-dir`: Directory to save extracted mentions.
"""
...
def pick_text(rec: Dict[str, Any]) -> tuple[str, str]:
    """Select best available text field; return (text, source_key)."""
    for k in (
        "text_for_speaker",      # newline-preserving (preferred)
        "text_bs4",              # old bs4 full-page text
        "parsed_text",           # legacy single text field
        "text_readability",      # readability prose
        "parsed_content_text",   # some raw sources
        "paragraph",             # fallback when scanning raw files
        "text",
    ):
        v = rec.get(k)
...
def load_interest_groups(csv_path: Path, *_, **__) -> List[InterestGroup]:
    ...
    for _, r in df.iterrows():
        ...
        # Do not silently replace filtered canonical names with gid; leave empty so we won't compile.
        canonical = name if keep_name(name) else ""
...
def _split_sentences_with_spans(text: str) -> List[Tuple[str, int, int]]:
    """Lightweight sentence splitter that also returns character spans (start,end) in the original text.
    We keep the same regex as _sentence_tokenize for consistency.
    """
    if not isinstance(text, str) or not text:
        return []
    sents: List[Tuple[str, int, int]] = []
    start = 0
    # Use the same boundary pattern to avoid drift
    for m in re.finditer(r"(?<=[.!?])\s+", text):
        end = m.start() + 0  # end at the whitespace; trim trailing spaces in slice below
        seg = text[start:m.start()]
        seg = seg.strip()
        if seg:
            # Recompute local trimmed offsets relative to full text
            # Find seg within the original window [start:m.start())
            # by skipping leading whitespace
            ws_prefix = 0
            while start + ws_prefix < len(text) and text[start + ws_prefix].isspace():
                ws_prefix += 1
            seg_start = start + ws_prefix
            seg_end = seg_start + len(seg)
            sents.append((seg, seg_start, seg_end))
        start = m.end()
    if start < len(text):
        tail = text[start:]
        seg = tail.strip()
...
def _make_acronym_pattern(acr: str) -> re.Pattern:
    """
    Build a case-sensitive pattern allowing optional dots/hyphens/slashes between letters,
    without adding an optional separator after the final token. Supports variants like A.F.L.-C.I.O.
    """
    parts: List[str] = []
...
def process_files(
    ...
    groups = load_interest_groups(interest_csv)
    ...
    def _process_record(rec: dict) -> List[Dict[str, Any]]:
        ...
        text, text_source = pick_text(rec)
        out: List[Dict[str, Any]] = []
        if not text:
            return out

        sent_spans = _split_sentences_with_spans(text)
        sentences = [s for (s, _, __) in sent_spans]
        for si, (sent_text, sent_start_abs, sent_end_abs) in enumerate(sent_spans):
            found: List[Tuple[str, str, bool, int, int]] = []

            # find all occurrences for both names and acronyms
            ...
            # de-dup by (org_id, start, end)
            found.sort(key=lambda t: (t[3], t[4], t[0], t[2]))
            dedup: List[Tuple[str, str, bool, int, int]] = []
            last_key = None
            for rec in found:
                key = (rec[0], rec[3], rec[4])
                if key != last_key:
                    dedup.append(rec)
                    last_key = key

            for org_id, variation, is_acro, start, end in dedup:
                p_start, p_end = max(0, si - 3), min(len(sentences) - 1, si + 3)
                para_start_abs = sent_spans[p_start][1]
                para_end_abs = sent_spans[p_end][2]
                mention_abs_start = sent_start_abs + start
                mention_abs_end = sent_start_abs + end
                out.append({
                    'org_id': org_id,
                    ...
                    'end_in_sentence': end,
                    'paragraph': " ".join(sentences[p_start:p_end + 1]),
                    # New: absolute offsets within the granule text
                    'mention_char_start': mention_abs_start,
                    'mention_char_end': mention_abs_end,
                    'paragraph_char_start': para_start_abs,
                    'paragraph_char_end': para_end_abs,
                    'timestamp': time.time(),
                })
        return out
...
def process_normalized_packages(
    ...
    def _process_row(row: dict) -> List[Dict[str, Any]]:
        ...
        # Prefer newline-preserving text_for_speaker; otherwise fallback via pick_text
